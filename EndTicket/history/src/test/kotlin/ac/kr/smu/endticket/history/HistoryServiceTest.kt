...
class HistoryServiceTest(
    ...
    fun given_eventResponseAndUserId_when_saveHistory_then_success(eventResponse: EventResponse, userId: Long){
        val type = if (eventResponse is TicketCompletedEventResponse) History.Type.TICKET else History.Type.IMAGINATION

        Mockito.`when`(repo.existsBySpecificIdAndType(eventResponse.id, type))
            .thenReturn(false)
        Mockito.`when`(ops.get(HISTORY_COUNT_REDIS_KEY))
            .thenReturn(null)
    ...
    @MethodSource("${HistoryTestParameters.PATH}#provideEventResponseAndUserId")
    fun given_eventResponseAndUserIdWithHistoryCountInRedis_when_saveHistory_then_saveHistoryAndUpdateHistoryCount(eventResponse: EventResponse, userId: Long){
        val type = if (eventResponse is TicketCompletedEventResponse) History.Type.TICKET else History.Type.IMAGINATION
        Mockito.`when`(repo.existsBySpecificIdAndType(eventResponse.id, type))
            .thenReturn(false)
        Mockito.`when`(ops.get(HISTORY_COUNT_REDIS_KEY))
        ...
        Mockito.verify(repo).save(mockAny())
        Mockito.verify(ops).set(HISTORY_COUNT_REDIS_KEY, when(type){
    ...
    @MethodSource("${HistoryTestParameters.PATH}#provideEventResponseAndUserId")
    fun given_eventResponseAlreadySaved_when_saveHistory_then_doNothing(eventResponse: EventResponse, userId: Long){
        val type = if (eventResponse is TicketCompletedEventResponse) History.Type.TICKET else History.Type.IMAGINATION
