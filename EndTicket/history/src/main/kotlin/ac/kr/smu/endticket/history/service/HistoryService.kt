...
class HistoryService(
    ...
    fun findHistories(
        ...
        type: History.Type,
        pageable: Pageable,
    ): HistorySlice<HistoryResponse> = repo.findAllByUserIdAndType(userId, type, pageable).map(History::toResponse)
    ...
    fun saveHistory(
        ...
        val (type, history) =
            when (eventResponse) {
                ...
                is ImaginationCompletedEventResponse ->
                    History.Type.IMAGINATION to
    ...
    private fun RedisTemplate<String, Any>.updateCountIfPresent(history: History) {
        val ops = opsForValue()
        val key = "${RedisConstant.HISTORY_COUNT_REDIS_PREFIX}::${history.userId}"
        val count = ops[key] as? HistoryCount ?: return

        val newCount =
            when (history) {
                is TicketHistory ->
                    HistoryCount(
                        count.ticketHistoryCount + 1,
                        count.ticketSwipeCount + history.swipeCount,
                        count.imaginationHistoryCount,
                    )

                is ImaginationHistory ->
                    HistoryCount(
                        count.ticketHistoryCount,
                        count.ticketSwipeCount,
                        count.imaginationHistoryCount + 1,
                    )
