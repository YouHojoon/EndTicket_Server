...
class TicketControllerTest
    ...
    constructor(
        ...
        fun given_ticketRequest_when_createTicket_then_responseCreatedTicket() {
            ...
            mvc
                .createTicket(TicketTestParameters.TICKET_REQUEST)
        ...
        fun given_ticketRequest_when_updateTicket_then_responseUpdatedTicket(ticket: Ticket) {
            ...
            mvc
                .updateTicket(TicketTestParameters.UPDATE_REQUEST, ticket.id)
        ...
        fun given_invalidId_when_updateTicket_then_responseExceptionResponseWithExpectedStatus(
            ...
            Mockito
                .`when`(service.updateTicket(TicketTestParameters.UPDATE_REQUEST, id, userId))
                .thenThrow(exception)

            mvc
                .updateTicket(TicketTestParameters.UPDATE_REQUEST, id, userId)
        ...
        fun given_id_when_swipeTicket_then_responseSwipedTicket() {
            ...
            Mockito
                .`when`(service.swipeTicket(ticket.id, TicketTestParameters.USER_ID))
                .thenReturn(
                    ticket.also { it.swipeAndCheckCompletion(TicketTestParameters.USER_ID) }.toResponse(),
                )

            mvc
                .swipeTicket(ticket.id)
                .andExpect(MockMvcResultMatchers.status().isOk)
        ...
        fun given_invalidId_when_swipeTicket_then_responseExceptionResponseWithExpectedStatus(
            ...
            Mockito
                .`when`(service.swipeTicket(id, userId))
                .thenThrow(exception)

            mvc
                .swipeTicket(id, userId)
                .andExpect(MockMvcResultMatchers.status().`is`(status))
                .expectExceptionResponse()
        }
        ...
        fun given_id_when_cancelSwipeTicket_then_responseSwipeCanceledTicket(ticket: Ticket) {
            ...
            Mockito
                .`when`(service.cancelSwipeTicket(ticket.id, TicketTestParameters.USER_ID))
                .thenReturn(
                    ticket
        ...
        fun given_invalidId_when_cancelSwipeTicket_then_responseExceptionResponseWithExpectedStatus(
            ...
            Mockito
                .`when`(service.cancelSwipeTicket(id, userId))
                .thenThrow(exception)

            mvc
                .cancelSwipeTicket(id, userId)
                .andExpect(MockMvcResultMatchers.status().`is`(status))
                .expectExceptionResponse()
        }
        ...
        fun given_userId_when_findIncompleteTickets_then_responseIncompleteTickets() {
            ...
            Mockito
                .`when`(service.findIncompleteTickets(TicketTestParameters.USER_ID))
                .thenReturn(tickets)
