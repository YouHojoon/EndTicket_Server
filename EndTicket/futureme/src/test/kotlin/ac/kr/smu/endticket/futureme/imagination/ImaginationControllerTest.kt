...
class ImaginationControllerTest
    @Autowired
    constructor(
        @MockBean
        ...
        fun given_requestAndUserId_when_createImagination_then_responseCreatedImagination(
            response: ImaginationResponse,
            request: ImaginationRequest,
        ) {
            Mockito
                .`when`(service.createImagination(request, ImaginationParameters.USER_ID))
                .thenReturn(response)
        ...
        fun given_requestExceedImaginationLimit_when_createImagination_then_responseExceptionResponseWithStatus409() {
            Mockito
                .`when`(service.createImagination(ImaginationParameters.REQUEST, ImaginationParameters.USER_ID))
        ...
        fun given_request_when_updateImagination_then_responseUpdatedImagination(
            ...
            mvc
                .updateImagination(request, response.id, ImaginationParameters.USER_ID)
                .andExpect(MockMvcResultMatchers.status().isOk)
                .andExpect(MockMvcResultMatchers.content().string(ObjectMapper().writeValueAsString(response)))
        ...
        fun given_invalidRequest_when_updateImagination_then_responseBindExceptionResponseWithStatus400(request: ImaginationRequest) {
            mvc
                .updateImagination(request, 1L, ImaginationParameters.USER_ID)
                .expectBindException()
        }
        ...
        fun given_invalidId_when_updateImagination_then_responseExceptionResponseWithExpectedStatus(
            ...
            mvc
                .updateImagination(request, id, userId)
        ...
        fun given_id_when_completeImagination_then_expectStatusCode204AndPublishImaginationCompletionEvent() {
            val id = 1L
            Mockito
                .`when`(service.completeImagination(id, ImaginationParameters.USER_ID))
                .then { eventService.publish(mockAny()) }

            mvc
                .completeImagination(id)
                .andExpect(MockMvcResultMatchers.status().isNoContent)
        ...
        fun given_invalidId_when_completeImagination_then_responseExceptionResponseWithExpectedStatus(
            ...
            Mockito
                .`when`(service.completeImagination(id, userId))
                .thenThrow(exception)

            mvc
                .completeImagination(id, userId)
                .andExpect(MockMvcResultMatchers.status().`is`(status))
                .expectExceptionResponse()
        }
    }
